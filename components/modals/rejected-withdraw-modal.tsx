...
export const RejectedWithdrawModal = () => {
  ...
  const onReject = async (e: MouseEvent) => {
    ...
    try {
      ...
      toast.success("Withdraw berhasil di reject");
      cookies.set("affiliateReq", "wrjct");
      onClose();
    } catch (error: any) {
      console.log("[ERROR_RJCT_WITHDRAW]:", error);
      toast.custom(
        (t) => (
          <ToastError label="Withdraw gagal di reject" error={error} t={t} />
        ),
        optionToast
      );
    }
