...
export const EditLabelModal = () => {
  const { isOpen, onClose, type, dataLabel } = useModal();
  ...
  const isModalOpen = isOpen && type === "edit-label";
  ...
  useEffect(() => {
    setInput({ name: dataLabel?.name ?? "", color: dataLabel?.color ?? "" });
  }, [isModalOpen]);

  return (
    <Modal
      title="Edit Label"
      description="Edit label disini"
      isOpen={isModalOpen}
      onClose={() => {
        onClose();
        handleReset();
